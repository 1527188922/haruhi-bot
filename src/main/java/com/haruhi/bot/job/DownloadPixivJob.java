    ...
    public static class downloadTask implements Runnable{
        ...
        public void run() {
            try {
                ...
                if(response == null){
                    Client.sendMessage(BotConfig.SUPER_USER,null, MessageTypeEnum.privat, MessageFormat.format("no data\n本次请求为null\n线程：{0}",Thread.currentThread().getName()), GocqActionEnum.SEND_MSG,true);
                    return;
                }
                ...
                Client.sendMessage(BotConfig.SUPER_USER,null, MessageTypeEnum.privat, MessageFormat.format("success\n本次添加{0}条\n线程：{1}",total,Thread.currentThread().getName()), GocqActionEnum.SEND_MSG,true);
            }catch (Exception e){
                Client.sendMessage(BotConfig.SUPER_USER,null, MessageTypeEnum.privat, MessageFormat.format("exception\n下载发生异常\n线程：{0}\n异常：{1}",Thread.currentThread().getName(),e.getMessage()), GocqActionEnum.SEND_MSG,true);
